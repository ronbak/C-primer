...
### 练习6.39
- a.  定义了一个返回值为int型，有两个const int 类型的参数；
不合法，这两个函数声明重复，在调用时会产生二义性！
- b.  定义了一个返回值为double型的函数；
不合法，由于两个函数仅仅只有返回值的类型不相同所以会造成二义性调用。
- c.  定义了一个返回值为double型的指针，并且拥有一个double型指针的参数；
合法。
...
### 练习6.41
- a.  非法，由于一个形参并没有默认值，所以必须要传递一个值；
- b.  合法；
- c.  合法，但是不符合初衷，当传递init(14, '*')时，参数'*'会被强制转换为int型然后赋给形参wd。
...
### 练习6.43
- 将内联函数的定义放入同文件中，这样在重复利用是会减少错误的产生；
> 对于某个给定的内联函数或者constexpr函数来说，它的多个定义必须**完全一致**。基于这个原因这两个函数通常定义在头文件中。


- 函数putValues()的声明既可以放在头文件中，也可以放在源文件中。在头文件中时，可以直接#include其所在的头文件中，在多个文件都需要利用该函数时，会很便利。如果仅有一个文件中对其进行调用，则可以仅放在源文件中。
...
### 练习6.49
> 在本次调用对应的重载函数集，集合中的函数称为**候选函数**  
候选函数的两个特征：  
①与被调用的函数同名；  
②其声明在调用点可见。

> 从候选函数中选出被这组实参调用的函数，这些新选出的函数称为**可行函数**  
可行函数的两个特征：  
①其形参数量与本次调用提供的实参数量相等；  
